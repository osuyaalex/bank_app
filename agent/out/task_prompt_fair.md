...
Everything below is exported from `lib/parsing/bank_alert.dart`, which your
file already imports. It is reproduced here so that nothing has to be looked
up: a candidate without tools cannot open that file, and a comparison where
one side has to guess the constructor is not a comparison.
...
class BankAlert {
  const BankAlert({
    required this.bank,        // String  -- a stable label, e.g. 'KUDA'
    required this.kind,        // AlertKind
    required this.channel,     // TxnChannel
    required this.narration,   // String  -- the descriptive part of the message
    this.amount,               // double?
    this.balanceAfter,         // double?
    this.occurredAt,           // DateTime?
    this.account,              // String?
    this.counterpartyKey,      // String?
    this.isReversal = false,   // bool
  });
}

/// Cleans a counterparty into a stable key: strips routing prefixes and
/// punctuation, upper-cases, and removes a trailing reference so two payments
/// to the same person do not key apart. Returns null for an empty result.
String? normaliseCounterparty(String? raw);
```

The names are exactly as written. `AlertKind`, not `TxKind`. `TxnChannel`,
not `TxChannel`. Getting them wrong produces a file that does not compile and
scores nothing.
