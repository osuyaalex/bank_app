...
class GoogleService{
  ...
  Future<bool?> fetchEmailsForToday(gmail.GmailApi gmailApi) async {
    ...
    do {
      try {
        ...
        shouldBreak = false; // Flag to determine if the loop should break
        for (var message in messages) {
          ...
          if (emailBody != null) {
            ...
            if (decodedBody.contains('Debit') && decodedBody.contains('Account Name') && decodedBody.contains('Description')) {
              ...
              await updateDailySpend(messageId!, plainText!);
  ...
  Future<bool?> authenticateAndFetchEmails() async {
    bool? shouldBreak;
    try {
      ...
      shouldBreak = await fetchEmailsForToday(gmailApi);
      return shouldBreak;
    } catch (e) {
