...
class _PendingPageState extends State<PendingPage> {
  ...
  Future<void> _fileTheObviousOnes() async {
    ...
    if (filed > 0 && mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('$filed sorted automatically.')));
      await _load();
    }
  }
  ...
  Widget _bulkBar() {
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            // A shortcut, dressed as one. A grey question mark beside "still
            // need a budget" read as a caution about work outstanding, which
            // is the opposite of what the bar is for: it is the fast way
            // through, not a warning about the slow one.
            const Icon(
            ...
            Expanded(
              child: Text(
                unsure > 0
                    ? 'Answer $unsure together, without opening each'
                    : 'Answer several together, without opening each',
                style: TextStyle(
                  fontSize: 12.5,
                  height: 1.35,
  ...
  Future<void> _acceptGuess(TransactionRecord txn, Category category) async {
    ...
    if (also > 0 && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '$also other payment${also == 1 ? "" : "s"} to '
  ...
  Future<void> _sort(TransactionRecord txn) async {
    ...
    final choice = await showCategoryPicker(
      ...
      ghostOptions: _guesses[txn.smsId]?.suggestedOptions ?? const [],
        ...
        child: Column(
          children: [
            if (_pending.isNotEmpty) ...[
              // No instruction panel. It said what `_intro` said, directly
              // above `_intro` saying it, and both explained a list that
              // explains itself: an amount, a name, and a budget to put it
              // in. The guide is still one tap away in the bar for anyone
              // who wants it.
              _stakes(),
              _bulkBar(),
            ],
  ...
  static const _guideSteps = [
    ...
    GuideStep(
      'The next payment to the same place is filed automatically, '
  ...
  /// What is waiting, in money as well as in rows.
  ///
  /// This replaced two paragraphs telling the user what the screen was for.
  /// The count alone does not say whether it is worth doing now -- six
  /// payments could be six hundred naira or six hundred thousand -- and the
  /// figure is the whole reason to open the screen.
  Widget _stakes() {
    final total = _pending.fold(0.0, (t, x) => t + (x.amount ?? 0));
    final n = _pending.length;
    return Padding(
      ...
      child: Align(
        ...
        child: Text(
          total > 0
              ? '$n payment${n == 1 ? '' : 's'} \u00b7 '
                    '$_currency${NumberFormat('#,###').format(total.round())} waiting'
              : '$n payment${n == 1 ? '' : 's'} waiting',
          style: TextStyle(
            fontSize: 13.5,
            fontWeight: FontWeight.w600,
            color: Colors.grey.shade800,
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.check_rounded, color: brandBlue, size: 30),
          ),
  ...
  Widget _card(TransactionRecord txn) {
    ...
    // no attempt at all.
    final Category? suggested = (guess != null && guess.categoryName.isNotEmpty)
          ...
          child: Row(
            children: [
              AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  shape: BoxShape.circle,
                  // Neutral, not orange. Orange is the colour of something
                  // having gone wrong, and nothing has: these are ordinary
                  // payments nobody has answered yet. Every row wearing a
                  // warning made a list of chores look like a list of faults.
                  color: saving
