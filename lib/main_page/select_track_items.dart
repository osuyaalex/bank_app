...
class _SelectTrackItemsState extends State<SelectTrackItems> {
  ...
  TextEditingController _textFieldValue = TextEditingController();
  ...
  Future<void> _setUserCurrency() async {
    try {
      ...
      if (permission == LocationPermission.denied ||
        ...
        // If denied, default to Naira (NGN)
        if (permission == LocationPermission.denied ||
            permission == LocationPermission.deniedForever) {
          print("Location permission denied. Defaulting to Naira (NGN).");
      ...
      // If permission is granted, proceed to get the user's location
      Position position = await Geolocator.getCurrentPosition(
          desiredAccuracy: LocationAccuracy.high);
      List<Placemark> placeMarks = await placemarkFromCoordinates(
          position.latitude, position.longitude);
      String countryCode = placeMarks.first.isoCountryCode ?? 'NG';
      if (countryCode == 'NG') {
        var format = NumberFormat.simpleCurrency(name: 'NGN');
        print("CURRENCY SYMBOL ${format.currencySymbol}"); // ₦
        print("CURRENCY NAME ${format.currencyName}"); // NGN
        ...
        var locale = Locale('en', countryCode);
      ...
      print("Failed to get location or country code: $e. Defaulting to Naira (NGN).");
      var format = NumberFormat.simpleCurrency(name: 'NGN');
      print("CURRENCY SYMBOL ${format.currencySymbol}"); // ₦
      print("CURRENCY NAME ${format.currencyName}"); // NGN
      setState(() {
  ...
  void initState() {
    ...
    // Add listener to controller to remove spaces after autocomplete
    _items.addListener(() {
      String currentText = _items.text;

      // Check if the last character is a space, and remove it
      if (currentText.endsWith(' ')) {
        _items.text = currentText.trimRight();  // Remove the space
        _items.selection = TextSelection.fromPosition(
          TextPosition(offset: _items.text.length),
                        ...
                        child: Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              _textFieldValue.text.isNotEmpty && keyword['workedOn'] == false
                                  ?
                              GestureDetector(
                                  onTap: (){
                                    setState(() {
                                      keyword['budgetSet'] = _textFieldValue.text;
                                      keyword['workedOn'] = true;
                                      _textFieldValue.clear();
                                    });
                                  },
                                  child: Icon(Icons.check_circle_sharp, color: Colors.green,size: 17,)
                              ):SizedBox()
                            ],
                          ),
                        ),
